...
const PRODUCTS = [
  { id: 1, name: "K-01 / Core Tee", category: "Shirts", price: 50, oldPrice: 64, badge: "BESTSELLER", color: "#d9d5ca", image: "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=1100&q=85", description: "A precise heavyweight tee with a relaxed shoulder and the Kimo mark at the chest." },
  { id: 2, name: "K-02 / After Dark", category: "Streetwear", price: 58, badge: "NEW DROP", color: "#24231f", image: "https://images.unsplash.com/photo-1503341504253-dff4815485f1?auto=format&fit=crop&w=1100&q=85", description: "An oversized black tee designed for late nights, long walks, and everything after." },
  { id: 3, name: "K-03 / Signal Tee", category: "Streetwear", price: 60, badge: "LIMITED", color: "#837d70", image: "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?auto=format&fit=crop&w=1100&q=85", description: "Small batch cotton jersey with the season's signal graphic across the back." },
  { id: 4, name: "K-04 / Studio Black", category: "Shirts", price: 52, color: "#171717", image: "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?auto=format&fit=crop&w=1100&q=85", description: "A clean everyday essential with a boxy fit and soft washed finish." },
  { id: 5, name: "K-05 / Transit Hoodie", category: "Streetwear", price: 92, badge: "LIMITED", color: "#2a2925", image: "https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&w=1100&q=85", description: "Heavy loopback fleece, built for the in-between hours." },
  { id: 6, name: "K-06 / Home Set", category: "Loungewear", price: 76, color: "#585149", image: "https://images.unsplash.com/photo-1618354691373-d851c5c3a990?auto=format&fit=crop&w=1100&q=85", description: "Soft brushed cotton, easy proportions, and zero effort required." },
];
...
export default function Home() {
  ...
  return <div className="kimo-app">
    ...
    <main id="home">
      ...
      <section className="shop-section page-width" id="shop"><div className="section-intro"><div><p className="kicker">01 / THE SHOP</p><h2>New arrivals</h2></div><p>Small batches, restocked monthly.<br />Once it's gone, it changes.</p></div><div className="shop-toolbar"><div className="category-tabs">{CATEGORIES.map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div><span>{filteredProducts.length} pieces</span></div><div className="product-grid">{filteredProducts.map((product) => <article className="product-card" key={product.id} onClick={() => setModalProduct(product)}><div className="product-image" style={{ backgroundColor: product.color }}><img src={product.image} alt={product.name} /><span className="product-badge">{product.badge ?? product.category}</span><span className="quick-view">Quick view <ArrowRight size={14} /></span></div><div className="product-meta"><div><h3>{product.name}</h3><p>{product.category}</p></div><div className="product-price">{product.oldPrice && <s>${product.oldPrice}</s>} ${product.price}</div></div></article>)}</div>{filteredProducts.length === 0 && <p className="empty-state">Nothing found. Try another search.</p>}</section>
